...
class ArmMovements():
    ...
    def paint_image(self, coordinates, image_x, image_y):
        ...
        if self.vertical == True:
            ...
            for contour in mapped_coordinates:
                ...
                for pair in contour:
                    ...
                    if start_x_abs == -1 and start_y_abs == -1:
                        start_y_abs = y_abs
                        start_z_abs = z_abs
                    current_path.append([self.hor_x_start, y_abs, z_abs, None, None, None, 50])
            ...
            for contour in mapped_coordinates:
                if self.travelled_dist >= self.reload_dist:
                    self.reload_brush(x_abs, y_abs, False)
                    self.travelled_dist = 0
                paths = []
                current_path = []
                start_x_abs = -1
                ...
                for num, current_path in enumerate(paths):
                    ...
                    # Do a reload between current_path and next one (if more than 1)
                    if num < len(paths)-1:
                        self.reload_brush(current_path[-1][0], current_path[-1][1], True)
                
            # Return to initial position
            self.initial_position()


    def reload_brush(self, orig_x, orig_y, low: bool):
        """
        ...
        :param low: False means stay above to page after, True means lower down so brush is touching the page.
        """
